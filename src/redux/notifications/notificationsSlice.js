...
export const fetchInvitationsAPI = createAsyncThunk(
  'notifications/fetchInvitationsAPI',
  async (_, { getState }) => {
    const { notifications } = getState();  // Lấy state hiện tại

    if (notifications.currentNotifications !== null) {
      console.log('Skipping API call, data already exists.');
      return notifications.currentNotifications;  // Tránh gọi API khi có dữ liệu
    }
